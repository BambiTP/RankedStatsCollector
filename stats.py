...
# ─── CONFIG ────────────────────────────────────────────────────────────────────
# By default, read the copied master CSV passed as first arg,
# otherwise fall back to outputs/main/combinedStatsMaster.csv
...
if __name__=='__main__':
    ...
    for players in matches.values():
        ...
        if not m: continue
        if m not in per_map:
            per_map[m]    = {'red_team':new_entry('Red'),'blue_team':new_entry('Blue')}
            map_results[m]= {'Games':0,'RedWins':0,'BlueWins':0}
